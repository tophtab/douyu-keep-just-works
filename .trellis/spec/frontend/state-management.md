...
- raw config, fans/gift, and yuba state should live in the focused `resource-*` modules and be re-exported through `resource-state.ts` when existing pages need the compatibility import surface.
...
When adding a new server resource, keep the API call in a Vue-owned composable or shared resource helper. If the resource is shared across pages, create or extend a focused `resource-*` module and re-export through `resource-state.ts` only when the existing import surface needs to stay stable. Do not introduce `window.DOUYU_KEEP_WEBUI_*` bridge state.
...
- Do not put every new shared resource back into `resource-state.ts`; keep ownership focused and let the facade compose the pieces.
