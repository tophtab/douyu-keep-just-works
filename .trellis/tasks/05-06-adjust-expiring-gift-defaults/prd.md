...
* User noticed the current expiring-gift default allocation mode still shows "按固定数量" because the default `model` is still `2`.
* User wants expiring-gift default allocation to be weight-based: the first/highest fan-medal room gets weight `1`, and every other room gets weight `0`.
...
* "第一个" means the first item in the synchronized fan-medal list as currently presented/processed by the app.
...
* Change expiring-gift default allocation mode from fixed-count (`model=2`) to weight-based (`model=1`).
* When creating or reconciling a default expiring-gift room list, assign weight `1` to the first fan-medal room and weight `0` to all other rooms.
* Keep existing user-saved expiring-gift room weights/counts unchanged during normalization/reconciliation.
...
* [ ] A newly created/default expiring-gift config uses `model=1` / "按权重".
* [ ] Default expiring-gift fan-room rows set the first synchronized fan-medal room weight to `1` and all other room weights to `0`.
* [ ] Existing user-saved expiring-gift weights/counts are preserved during reconciliation.
