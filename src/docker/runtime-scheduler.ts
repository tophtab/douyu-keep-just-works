...
interface DockerTaskSchedulerDeps {
  logSystem: (message: string) => void
  taskLoggers: Record<TaskType, (message: string) => void>
...
export class DockerTaskScheduler {
  ...
  async runTaskWithLock(
    ...
    if (this.activeRuns[type]) {
      if (options.onBusy === 'skip') {
        this.deps.taskLoggers[type](options.busyMessage)
        return false
      }
  ...
  reconcileTaskJobs(prevConfig: DockerConfig | null, nextConfig: DockerConfig, hasCookieSource: boolean): TaskReloadSummary {
    ...
    for (const type of TASK_TYPES) {
      ...
      const nextShouldRun = hasCookieSource && isTaskEnabled(nextTaskConfig)
  ...
  logTaskReloadSummary(reason: 'startup' | 'cookie_saved' | 'tasks_saved' | 'ui_saved' | 'medal_synced', summary: TaskReloadSummary): void {
    ...
    if (reason === 'cookie_saved') {
      if (!parts.length) {
        this.deps.logSystem('登录凭证已更新，现有任务继续运行，无需重启')
        return
      }
      this.deps.logSystem(`登录凭证已更新，仅调整受影响任务：${parts.join('；')}`)
      return
    }

    if (reason === 'tasks_saved') {
      if (!parts.length) {
        this.deps.logSystem('任务配置已更新，未重启无关任务')
        return
      }
      this.deps.logSystem(`任务配置已更新，仅调整受影响任务：${parts.join('；')}`)
      return
    }

    if (!parts.length) {
      this.deps.logSystem('粉丝牌列表变化，但相关任务配置无需重载')
      return
    }

    this.deps.logSystem(`粉丝牌列表变化，仅调整受影响任务：${parts.join('；')}`)
  }
  ...
  private createTaskRunnerDeps(): RuntimeTaskRunnerDeps {
    return {
      taskLoggers: this.deps.taskLoggers,
      resolveCookieForUrl: this.deps.resolveCookieForUrl,
      refreshCookieSourceAfterFailure: this.deps.refreshCookieSourceAfterFailure,
      runAndInvalidateStatusCache: this.deps.runAndInvalidateStatusCache,
  ...
  private startScheduledTask(options: {
    type: TaskType
    label: string
    cron: string
    runTask: () => Promise<void>
    ...
    const { type, label, cron, runTask, summary } = options
    const logger = this.deps.taskLoggers[type]
    const run = async () => {
    ...
    this.statuses[type].nextRun = job.nextDate().toISO()
    this.deps.logSystem(`${label}已启动, cron: ${cron}, 下次执行: ${formatScheduleForLog(this.statuses[type].nextRun)}${summary ? `, ${summary}` : ''}`)
  }
  ...
  private startTask(type: TaskType, config: DockerConfig): void {
    ...
    if (!taskConfig || !isTaskEnabled(taskConfig) || !cron) {
      return
    }
    ...
    this.startScheduledTask({
      ...
      summary: getTaskScheduleSummary(type, taskConfig),
    })
  }
}
