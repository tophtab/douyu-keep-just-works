...
test('Docker WebUI is Vue-only and served as Vite static Docker assets', () => {
  ...
  assert.match(resources, /export async function syncFans/)
  assert.match(resources, /export async function loadFansList/)
  assert.match(resources, /export async function loadFansStatus/)
  assert.match(resources, /export async function loadYubaStatus/)
  assert.match(resources, /function createResourceRequest\(\)/)
  assert.match(resources, /pending:\s*null/)
  assert.match(resources, /fetchedAt:\s*0/)
  assert.match(resources, /requestSeq:\s*0/)
  assert.match(resources, /trackResourceRequest\(resource,\s*requestSeq,\s*pending\)/)
  ...
  assert.match(resources, /\/api\/yuba\/status/)
  ...
  assert.match(collect, /refreshOverviewSurface\('collect', false\)/)
  assert.match(keepalive, /watch\(\[sharedRawConfig, managed, sharedFansStatus, sharedOverview/)
  assert.match(keepalive, /refreshOverviewSurface\('keepalive', false\)/)
  assert.match(double, /watch\(\[sharedRawConfig, managed, sharedFansStatus, sharedOverview/)
  assert.match(double, /refreshOverviewSurface\('double-card', false\)/)
  assert.match(expiring, /watch\(\[[\s\S]*sharedGiftStatus/)
  assert.match(expiring, /refreshOverviewSurface\('expiring-gift', false\)/)
...
test('Docker task scheduling uses shared task metadata for inventory facts', () => {
  ...
  assert.match(scheduler, /private async runScheduledTask/)
