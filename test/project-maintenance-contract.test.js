...
test('Docker WebUI remains Vue-only without legacy bridge files', () => {
  ...
  const formField = readRepoFile('src/docker/webui/components/FormField.vue')
  ...
  assert.match(formField, /class="field-block"/)
  assert.match(formField, /class="field-label"/)
  assert.match(formField, /<slot \/>/)
...
test('Docker WebUI resource and page ownership stays in focused Vue modules', () => {
  ...
  assert.match(formField, /helperText/)
  ...
  assert.match(loginConfigPage, /<FormField[\s\S]*v-model="mainCookie"[\s\S]*v-model="yubaCookie"[\s\S]*v-model="passportCookie"/)
  assert.match(loginConfigPage, /<FormField[\s\S]*v-model="cookieCloud\.endpoint"[\s\S]*v-model="cookieCloud\.uuid"[\s\S]*v-model="cookieCloud\.password"/)
  ...
  assert.match(keepalivePage, /<FormField[\s\S]*keepalive-model[\s\S]*<select/)
  assert.match(doublePage, /<InlineFeedback>[\s\S]*<DataContent content-id="double-table-wrap"/)
  assert.match(expiringPage, /<FormField[\s\S]*expiring-threshold-hours[\s\S]*type="number"[\s\S]*<DataContent content-id="expiring-backpack-wrap"/)
...
test('manual passport cookie stays masked outside raw config and is saved from login config UI', () => {
  ...
  assert.match(loginConfigPage, /<FormField[\s\S]*v-model="passportCookie"/)
