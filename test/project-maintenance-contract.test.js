...
test('Docker WebUI remains Vue-only without legacy bridge files', () => {
  ...
  const dataContent = readRepoFile('src/docker/webui/components/DataContent.vue')
  const emptyState = readRepoFile('src/docker/webui/components/EmptyState.vue')
  const inlineFeedback = readRepoFile('src/docker/webui/components/InlineFeedback.vue')
  const numberField = readRepoFile('src/docker/webui/components/NumberField.vue')
  const selectField = readRepoFile('src/docker/webui/components/SelectField.vue')
  const textareaField = readRepoFile('src/docker/webui/components/TextareaField.vue')
  const textField = readRepoFile('src/docker/webui/components/TextField.vue')
  ...
  const uiTypes = readRepoFile('src/docker/webui/ui-types.ts')
  ...
  assert.match(actionBar, /ActionBarAction/)
  assert.match(actionBar, /emit\('action', action\.id\)/)
  assert.match(actionBar, /:disabled="action\.disabled \|\| action\.loading"/)
  assert.doesNotMatch(actionBar, /action: \[index: number\]/)
  assert.match(uiTypes, /export interface ActionBarAction/)
  assert.match(uiTypes, /export interface FieldOption/)
  assert.match(dataContent, /<EmptyState v-if="!show"/)
  assert.match(dataContent, /class="table-shell"/)
  assert.match(emptyState, /class="empty"/)
  assert.match(inlineFeedback, /class="status-box"/)
  assert.match(selectField, /FieldOption/)
  assert.match(numberField, /type="number"/)
  assert.match(textField, /<input/)
  assert.match(textareaField, /<textarea/)
...
test('Docker WebUI resource and page ownership stays in focused Vue modules', () => {
  ...
  assert.match(actionBar, /action\.id/)
  assert.match(actionBar, /loadingLabel/)
  assert.match(dataContent, /contentId/)
  assert.match(emptyState, /empty-with-action/)
  assert.match(inlineFeedback, /aria-live/)
  assert.match(selectField, /options: FieldOption\[\]/)
  assert.match(numberField, /'update:modelValue': \[value: FieldValue\]/)
  assert.match(textareaField, /spellcheck/)
  assert.match(textField, /placeholder/)
  assert.match(uiTypes, /export interface StatusCell/)
  ...
  assert.match(overviewPage, /<InlineFeedback v-if="overviewFansFeedbackText"/)
  assert.match(overviewPage, /<DataContent[\s\S]*showOverviewFansTable/)
  assert.match(loginConfigPage, /<TextareaField[\s\S]*v-model="mainCookie"[\s\S]*v-model="yubaCookie"[\s\S]*v-model="passportCookie"/)
  assert.match(loginConfigPage, /<TextField[\s\S]*v-model="cookieCloud\.endpoint"[\s\S]*v-model="cookieCloud\.uuid"[\s\S]*v-model="cookieCloud\.password"/)
  assert.match(logsPage, /<ActionBar[\s\S]*id: 'refresh'[\s\S]*id: 'clear'/)
  assert.match(logsPage, /<EmptyState v-if="!formattedLogs\.length" text="暂无日志"/)
  assert.match(keepalivePage, /<SelectField[\s\S]*keepalive-model/)
  assert.match(doublePage, /<InlineFeedback>[\s\S]*<DataContent content-id="double-table-wrap"/)
  assert.match(expiringPage, /<NumberField[\s\S]*expiring-threshold-hours[\s\S]*<DataContent content-id="expiring-backpack-wrap"/)
  assert.match(yubaPage, /<DataContent content-id="yuba-table-wrap"/)
...
test('manual passport cookie stays masked outside raw config and is saved from login config UI', () => {
  ...
  assert.match(loginConfigPage, /<TextareaField[\s\S]*v-model="passportCookie"/)
  assert.match(loginConfigPage, /input-id="cookie-cloud-endpoint"[\s\S]*label="服务器地址"/)
